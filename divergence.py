...
def find_rsi_divergence(df, min_bars=30):
    ...
    for i in range(min_bars, len(df)):
        ...
        past_high = df.iloc[i - min_bars:i]['Close'].max()
        past_low = df.iloc[i - min_bars:i]['Close'].min()

        past_rsi_high = df.iloc[i - min_bars:i]['RSI'].max()
        past_rsi_low = df.iloc[i - min_bars:i]['RSI'].min()
        ...
        if past_rsi_high > 70 and curr_price > past_high and curr_rsi < past_rsi_high:
            df.at[df.index[i], 'Bearish_Divergence'] = curr_price
        else:
            df.at[df.index[i], 'Bearish_Divergence'] = 0


        # Bullish divergence (price lower low, RSI higher low)
        if past_rsi_low < 30 and curr_price < past_low and curr_rsi > past_rsi_low:
            df.at[df.index[i], 'Bullish_Divergence'] = curr_price
        else:
            df.at[df.index[i], 'Bullish_Divergence'] = 0
